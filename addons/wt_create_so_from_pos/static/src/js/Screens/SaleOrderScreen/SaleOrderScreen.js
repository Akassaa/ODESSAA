...
export class SaleOrderScreen extends ControlButtonsMixin(Component) {
	...
	async _onClickSaleOrder(clickedOrder) {
	  const { confirmed, payload: selectedOption } = await this.popup.add(SalesOrderPopup, {
		title: _t('Commande') + '  ' + clickedOrder.name,
		list: [
		  {
			id: "1",
			label: _t("Confirmer la commande"),
			item: 'confirm',
			icon: 'fa fa-check-circle',
		  },
		  {
			id: "2",
			label: _t("Annuler la commande"),
			item: 'cancel',
			icon: 'fa fa-close'
		  },
		],
		...
		if (selectedOption === 'confirm') {
		  if (clickedOrder.state !== 'sale') {
			...
			await this.popup.add(ConfirmPopup, {
			  title: _t('Confirmé!'),
			  body: _t('La commande') + clickedOrder.name + ' ' + _t(' est confirmée.'),
			});
		  } else {
			await this.popup.add(ConfirmPopup, {
			  title: _t('Déjà confirmé'),
			  body: _t(
				'Cette commande  est déjà à l’état confirmé !'
			  ),
			});
		  }
		  ...
		  if (clickedOrder.state !== 'cancel') {
			...
			await this.popup.add(ConfirmPopup, {
			  title: _t('Annulé!'),
			  body: _t('La commande a été ') + clickedOrder.name + ' ' + _t('annulée"'),
			});
		  } else {
			await this.popup.add(ConfirmPopup, {
			  title: _t('Déjà annulé'),
			  body: _t(
				'Cette commande est déjà à l’état Annulé !'
			  ),
			});
		  }
		}
